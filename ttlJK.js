...
async function kcjk(giftId){
    ...
    let nm = {
      ...
      bb= data.match(/","exchangeLimit":.+/)     
           ...
           if(result.gifts.stockAmount == "0" ){
            $.log(`${result.gifts.giftName} 需要积分：${result.gifts.price} 库存：${result.gifts.stockAmount}`)
           } else {
            if ($.isNode()) await notify.sendNotify( `${result.gifts.giftName} 需要积分：${result.gifts.price} 库存：${result.gifts.stockAmount}`);
           }
   }catch(e) {
             $.logErr(e, response);
         } finally {
